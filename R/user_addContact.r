...
#'}
addContact <- function(mendeley_cred = NULL, mendeley_user_id = NULL, ..., curl = getCurlHandle()) {
    ...
    add_url <- paste("http://api.mendeley.com/oapi/profiles/contacts/", mendeley_user_id,
        "/", sep = "")
    add <- OAuthRequest(mendeley_cred, add_url, , "POST")
    return(fromJSON(add))
}
...
# OAuthRequest(mendeley_cred, add_url, , 'POST')
