...
user_download_file <- function(mendeley_cred = NULL, doc, file = NA, binary = NA,
    ...
    if (is.na(file)) {
        ...
        if (length(id) == 1)
            file <- id else return(lapply(id, function(i) downloadFile(cred, doc, i, binary, ...,
            curl = curl)))
    }
    if (length(file) > 1)
...
if (FALSE) {
    mg <- user_create_group(cred, "MyGroup")
    user_delete_group(cred, mg)
    doc <- list(type = "Book", title = "MySQL Cookbook", authors = I("DuBois"))
    user_create_document(cred, doc)
